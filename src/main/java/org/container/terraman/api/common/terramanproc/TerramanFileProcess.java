...
public class TerramanFileProcess {
     ...
     * @return the String
     */
    public String tfCreateWithWriteNhn(FileModel fileModel, File file) {
        String resultCode = Constants.RESULT_STATUS_SUCCESS;
        // BufferedWriter 생성 및 쓰기설정(파일 덮어쓰기 - false)
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(file, false));) {
            // 파일 쓰기
            ...
            String jsonString = gson.toJson(fileModel);
            jsonString = jsonString.replaceAll("[,]", "");
            jsonString = jsonString.replace("\"nhnTenantName\":", "tenant_id =");
            jsonString = jsonString.replace("\"nhnPassword\":", "password =");
            jsonString = jsonString.replace("\"nhnAuthUrl\":", "auth_url =");
            jsonString = jsonString.replace("\"nhnUserName\":", "user_name =");
            jsonString = jsonString.replace("\"nhnRegion\":", "region =");

            writer.write(TerramanConstant.PREFIX_PROVIDER_NHN + "\n\n" + "provider \"openstack\" " + jsonString);
                ...
                if(fileFlag) {
                    switch(provider.toUpperCase()) {
                        ...
                        case Constants.UPPER_NHN :
                            resultCode = this.tfCreateWithWriteNhn(fileModel, file);
                            break;
